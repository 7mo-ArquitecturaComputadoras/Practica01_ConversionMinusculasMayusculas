...
\begin{figure}[H]
    ...
    \resizebox{0.7\textwidth}{!}{%
        \begin{tikzpicture}[
            ...
            % --- Nodo Fin (Derecha, alineado con es0) ---
            \node[startstop, right=2.2cm of es0]     (fin)     {Fin};

            % --- Enlaces de la ruta principal ---
            ...
            % --- Salida al Fin (Carril Derecho Superior) ---
            \draw[arr] (es0)     -- node[above, font=\scriptsize]{Sí} (fin);

            % --- Bypass de mayúsculas/símbolos (Carril Derecho Inferior) ---
            \draw[arr] (rango.east) -- ++(1.5,0) |- node[near start, above, font=\scriptsize]{No} (inc.east);

            % --- Bucle de Retorno (Carril Izquierdo, libre de cruces) ---
            \draw[arr] (inc.west) -- ++(-1.8,0) |- (leer.west);
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{imagenes/memoria_01.png}
    \caption{Ventana Memory de Visual Studio: cadena \texttt{"Hola Mundo"} antes de la conversión}
    \label{fig:memoria_antes}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{imagenes/memoria_02.png}
    \caption{Ventana Memory de Visual Studio: cadena \texttt{"HOLA MUNDO"} después de la conversión}
    \label{fig:memoria_despues}
\end{figure}

% ============================================================
\section{Repositorio GitHub}

\url{https://github.com/7mo-ArquitecturaComputadoras/Practica01\_ConversionMinusculasMayusculas}
