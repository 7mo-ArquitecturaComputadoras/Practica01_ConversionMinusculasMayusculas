...
% ============================================================
\section{¿Cómo funciona la conversión?}

El siguiente diagrama muestra el recorrido completo del programa para cada carácter de la cadena:
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % "No" branch de rango hacia inc (sin convertir)
        \draw[arr] (rango.west) -- ++(-1.5,0) |-
              node[near start, above, font=\scriptsize]{No} (inc.west);

        % Flecha de retorno de inc hacia leer
...
Para comprender mejor cómo opera el programa, se muestra a continuación la traza completa para la cadena \texttt{"Hola Mundo"}.

\begin{table}[H]
    \centering
    \caption{Traza de ejecución carácter por carácter}
    \begin{tabular}{@{}ccccp{4cm}@{}}
        \toprule
        \textbf{Paso} & \textbf{Carácter} & \textbf{ASCII} & \textbf{¿Minúscula?} & \textbf{Acción} \\
        \midrule
        1  & \texttt{H} &  72 & No  & Se conserva \\
        2  & \texttt{o} & 111 & Sí  & $111 - 32 = 79$ → \texttt{O} \\
        3  & \texttt{l} & 108 & Sí  & $108 - 32 = 76$ → \texttt{L} \\
        4  & \texttt{a} &  97 & Sí  & $97  - 32 = 65$ → \texttt{A} \\
        5  & \texttt{ } &  32 & No  & Se conserva (espacio) \\
        6  & \texttt{M} &  77 & No  & Se conserva \\
        7  & \texttt{u} & 117 & Sí  & $117 - 32 = 85$ → \texttt{U} \\
        8  & \texttt{n} & 110 & Sí  & $110 - 32 = 78$ → \texttt{N} \\
        9  & \texttt{d} & 100 & Sí  & $100 - 32 = 68$ → \texttt{D} \\
        10 & \texttt{o} & 111 & Sí  & $111 - 32 = 79$ → \texttt{O} \\
        11 & \texttt{0} &   0 & ---  & Fin de cadena \\
        \bottomrule
    \end{tabular}
\end{table}

El resultado final almacenado en memoria es \texttt{"HOLA MUNDO"}.
